...
export const StyledButton = styled.button<Omit<CircleButtonProps, 'children' | 'ref'>>`
  display: flex;
  ...
  font-size: ${({ size }) => (size === 'large' ? `${theme.sizes.fontSizes[1]}` : `${theme.sizes.fontSizes[0]}`)}px;
  font-weight: ${theme.sizes.fontWeight[3]};
  line-height: ${theme.sizes.fontSizes[6]}px;
  color: ${({ view }) => (view === 'primary' ? `${theme.colors.white}` : `${theme.colors.primary}`)};
  cursor: pointer;
  outline: none;

  &:disabled {
    background-color: ${({ view }) => (view === 'primary' ? `${theme.colors.grey}` : 'transparent')};
